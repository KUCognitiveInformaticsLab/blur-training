...
if __name__ == "__main__":
    ...
    for model_name in tqdm(model_names, desc="models"):
        ...
        for layer_id, layer in tqdm(enumerate(layers), "plotting (each layer)"):
                ...
                for filter_id in range(num_filters + 1):
                    ...
                    if num_dim == 2:
                        plt.scatter(
                            x=target[0],
                            y=target[1],
                            label=f"l{label_id}-f{filter_id}",
            ...
            plt.title(
                f"{analysis}, p={perplexity}, i={n_iter}, l={label_id}, n={num_images}, {num_classes}, {rename_model_name(arch=arch, model_name=model_name)}, {layer}",
                fontsize=8,
            )
            # fig.tight_layout()
            plot_file = f"{analysis}_{num_dim}d_p{perplexity}_i{n_iter}_l{label_id}_n{num_images}_{num_classes}-class_{model_name}_{layer}.png"
